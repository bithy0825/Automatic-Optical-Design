...
#
# 用法:
#   bash scripts/len_batch.sh <输入目录> <输出目录>
#
...
if [ $# -ne 2 ]; then
  cat <<'EOF'
用法: bash scripts/len_batch.sh <输入目录> <输出目录>
示例:
  bash scripts/len_batch.sh "E:/Users/资源/Photographic lenses - prime" out/len
  bash scripts/len_batch.sh /data/lenses/len /data/configs/len
EOF
  exit 2
fi
...
converter="$root/optimization/len2toml.py"
